...
namespace AutoAssistantAppDataLibrary.Helpers
{
    public static class CreateAccountHelper
    {
        ...
        public static async Task<AccountDataItem> CreateAccountLocally(string username, string hashedPassword, long accountId, int deviceId)
        {
            try
            {
                AccountDataItem account = account = await AccountsManager.CreateAccount(username, hashedPassword, accountId, deviceId, true, true, true);
